...
- **函数图像**:`plot2d` / `plot3d` 代码块生成可交互的 2D 曲线与 3D 曲面(含隐式方程 `f(x,y)=0` / `f(x,y,z)=0`,圆锥曲线、球面、环面),一张图能叠多个曲面/等值面并支持半透明,鼠标缩放 / 旋转,自己实现、零依赖;
...
同一个 `plot3d` 里可以叠**多个**曲面/等值面(最多 8 个),它们会一起按深度排序,所以互相遮挡是对的:
...
- **surface nets 的边界**:曲面正好贴到包围盒的面上时那一圈会缺面片,把范围开大一点即可;
...
| `grid=n` | 显式 46 / 隐式 32 | 3D 网格密度 |
| `alpha=a` | 多曲面 0.62 / 单曲面 1 | 3D 曲面透明度(0.05~1),`opacity=` 也行;另见上文「半透明」 |
| `n=n` | 900 | 2D 采样点数 |
