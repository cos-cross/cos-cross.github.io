...
- **函数图像**:`plot2d` / `plot3d` 代码块生成可交互的 2D 曲线与 3D 曲面(含隐式方程 `f(x,y)=0` / `f(x,y,z)=0`,`sphere(球心, 半径)` 直接画球面,圆锥曲线、球面、环面),一张图能叠多个曲面/等值面并支持半透明,还能画空间中的**线段**、**平面多边形**和**晶格骨架**(`centers` / `links(d)`),鼠标缩放 / 旋转,自己实现、零依赖;
