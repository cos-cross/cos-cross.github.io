...
绘制代码只在**含图像的页面**加载(`layout.ejs` 靠正文里有没有 `class="plot"` 判断),其他页面完全不受影响。脚本 URL 带 `?v=` 版本号(按文件内容自动算的指纹,见「静态资源的缓存」一节),payload 结构变化时用来防缓存。
...
**公式里的下划线是安全的** —— 公式在 Markdown **之前**就被抽走了(见上文),所以 `$D_1$`、`\sigma_x` 都不会被当成斜体。这是把抽取挪到前面顺带修好的:以前 `$D_1$区…$D_2$` 这种连着写的式子会被 Markdown 的下划线配对规则吃掉一部分。

### 标题里的公式:为什么目录会"重影",阅读时长会虚高

KaTeX 渲染一个公式会吐出**两层文字**:`<span class="katex-mathml">`(隐藏层,MathML 字形 + TeX 注解,给屏读器)
和 `<span class="katex-html">`(可见层,真正画出来的字形)。这在正文里没问题,但**任何"把正文抽成纯文本"的下游都会把两层一起串起来**:

- **侧边目录**:标题写成 `### $D_3$、$D_4$区:完整算一遍` 时,目录里那行变成 `D3D_3D3、D4D_4D4区:完整算一遍` —— 一个 `$D_3$` 贡献了 `D` `3` `D_3` `D` `3` 五段。Hexo 的 `toc()` 最后是用 htmlparser2 的 `textContent()` 抽标题的,两层一视同仁;(标题的锚点 id 不受影响 —— 它是 Markdown 渲染时用占位 span 生成的,一直是干净的。)
- **预计阅读时长**:除公式隐藏层外,plot 的内联 `<script type="application/json">` 数据和 `<style>` 正文也全被当成"字"数进去 —— 那篇数学笔记一度被算成 **99 分钟**(剥掉这些之后约 20 分钟)。

所以有了 `tools/readable-html.cjs`(Hexo 侧在 `scripts/readable-html.js` 注册成 `readable_html` helper):去掉 KaTeX 隐藏层、去掉脚本/样式正文、去掉零宽字符,**再**交给 Hexo 的 `strip_html`。用到它的地方:`post.ejs`(目录 + 时长)、`post-card.ejs` / `archive.ejs`(时长 + 摘要)、`_partial/head.ejs`(meta description)。

两个实现上的细节:

- 隐藏层是**按标签配平**扫掉的,不是正则 `<span class="katex-mathml">.*?</span>` —— 万一以后层里出现嵌套 `<span>`,非贪婪匹配会提前收工,把半截标签留在正文里;配平失败时宁可不清,也不能吃掉后文;
- 正文 HTML 本身**不动**,隐藏层仍留在页面里(屏读器和"复制成 TeX"还要用),只是抽纯文本之前先剥掉。

顺带一个坑:侧边目录只收 **1–3 级**标题(`post.ejs` 里的 `max_depth: 3`),所以 `####` 往下的标题**不会**出现在目录里 —— 把章节整体降级时要留意这个。
...
**样式改了、页面却没变** —— 这个坑真踩过:GitHub Pages 给 CSS 的缓存是 `max-age=600`
(CDN 还会再存一层),于是出现「HTML 已经更新、`style.css` 还是十分钟前的」这种状态:
徽章的内联样式明明已经上线,浏览器还在用旧 CSS,看起来就像"改了没生效"。

所以 `style.css` / `main.js` / `plot.js` / `katex.min.css` 的链接现在都挂一个
**按文件内容算出来的指纹**(`tools/asset-version.cjs` + Hexo helper `asset_v`):

```html
<link rel="stylesheet" href="/css/style.css?v=afd53ca3">
```

- 内容一变,URL 就变,缓存自然失效 —— **改样式不用再让人强刷**;
- 内容没变则 URL 不变,继续吃缓存(不是每次构建都换号);
- 读的是源文件(主题 `source/` 优先),同一次构建里只读一遍;
- 文件找不到时只是不加版本号并在日志里提醒,不会让构建挂掉。

以前 `plot.js` 是手写的 `?v=12`,现在也归它管了,改绘图代码不用记得手动加号。
`tools/check.mjs` 校验链接时会先剥掉查询串,所以带 `?v=` 的链接不会被误判成死链。
