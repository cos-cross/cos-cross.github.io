...
- **函数图像**:`plot2d` / `plot3d` 代码块生成可交互的 2D 曲线与 3D 曲面(含隐式方程 `f(x,y)=0` / `f(x,y,z)=0`,圆锥曲线、球面、环面),鼠标缩放 / 旋转,自己实现、零依赖;
...
单独的点用 `point(x, y)` / `point(x, y, z)`,后面可以跟标签,坐标是常量表达式:
